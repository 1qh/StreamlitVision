...
WHITE_BLACK_THRESHOLD = 384


class ColorClassifierAnnotator:
  def __init__(
    self,
    clf: ColorClassifier = ColorClassifier(),  # noqa: B008
    text_color: Color = Color.BLACK,
    text_scale: float = 0.5,
    text_thickness: int = 1,
    text_padding: int = 10,
  ) -> None:
    self.naive: bool = False
  ...
  def annotate(
    ...
    for center, bb in zip(centers, xyxy, strict=False):
      x = center[0]
      y = center[1]
...
class FpsAnnotator:
  def __init__(
    self,
    text_anchor: Point = Point(x=50, y=20),  # noqa: B008
    text_color: Color = Color.BLACK,
    text_scale: float = 0.5,
    text_thickness: int = 1,
    text_padding: int = 1,
  ...
  def annotate(self, scene: i8, _) -> i8:  # noqa: ANN001
...
class CountAnnotator:
  def __init__(
    ...
    text_anchor: Point = Point(x=50, y=12),  # noqa: B008
    text_scale: float = 0.5,
    text_thickness: int = 1,
    text_padding: int = 1,
  ) -> None:
      ...
      for i, c in enumerate(np.bincount(class_id)):
        if c:
          ...
          draw_text(
            ...
            text_color=Color.WHITE if np.sum(bg.as_bgr()) < WHITE_BLACK_THRESHOLD else Color.BLACK,
            text_scale=self.text_scale,
            text_thickness=self.text_thickness,
            text_padding=self.text_padding,
            background_color=bg,
...
class AreaAnnotator:
  ...
  def annotate(self, scene: i8, detections: Detections) -> i8:
    xyxy = detections.xyxy.astype(int)
    centers = (xyxy[:, [0, 1]] + xyxy[:, [2, 3]]) // 2
    for a, c in zip(detections.area.astype(int), centers, strict=False):
      draw_text(
        scene=scene,
        text=str(a)[:-1],
...
class LineAndZoneAnnotator:
  def __init__(
    ...
    self.zs: list[PolygonZone] = [PolygonZone(polygon=np.array(p)) for p in self.draw.zones]
  ...
  def annotate(self, scene: i8, detections: Detections) -> i8:
    ...
    for z, zone in zip(self.zs, self.zones, strict=False):
      z.trigger(detections)
      zone.annotate(scene)

    return scene

  def update(self, f: i8) -> None:
    ...
    self.zs = [PolygonZone(polygon=(z.polygon * scale).astype(int)) for z in self.origin_zs]
    for i, z in enumerate(self.zs):
      self.zones[i].zone = z
