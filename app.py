...
markdown(f'<style>{Path("style.css").read_text(encoding="utf-8")}</style>', unsafe_allow_html=True)
...
custom = [i for i in Path().glob('*.pt') if i not in pretrained]

use_custom = m_tab.toggle(
  'Custom weight',
  disabled=not custom,
  help=f'{"" if custom else "No custom weight found "}in current folder',
)
...
options: dict[str, Any] = {'conf': conf, 'iou': iou}
if len(classes):
  options |= {'classes': classes}
if tracker:
  options |= {'tracker': f'{tracker}.yaml', 'persist': True}
...
if enable_camera:
  ...
  if vid_tab.toggle('Annotate from image'):
    ...
    if vid_tab.toggle('Shoot') and (shot := camera_input('Shoot')):
      model.predict_image(shot)
      canvas_bg = Image.open(shot)
  draw = canvas2draw(wh, canvas_bg, key=str(wh[0]), line=bool(tracker)) if task in {'detect', 'segment'} else None
...
if file:
  if 'image' in file.type:
    ...
    Path(source).write_bytes(file.read())
...
""")
    draw = (
      canvas2draw(wh, first_frame(source), key=source, line=bool(tracker)) if task in {'detect', 'segment'} else None
    )
  ...
  match k:
    case 'ColorClassifier':
      ...
      if len(color_names) > 0:
        ...
        for c, rgb in zip(clf.names, clf.rgb, strict=False):
...
while running:
  ...
  t1.image(from_ann, use_container_width=True, channels='BGR')
  t2.image(fallback, use_container_width=True, channels='BGR')
  if source:
    count += 1
...
def stream(stream_key: str, callback: VideoFrameCallback) -> None:
  webrtc_streamer(
    key=stream_key,
...
def cam(frame: VideoFrame) -> VideoFrame:
...
def cam_adapt(frame: VideoFrame) -> VideoFrame:
  fr = frame.to_ndarray(format='rgb24')
  global _shape  # noqa: PLW0603
...
if source:
  ...
  if exp_tab.button('Export config to JSON'):
    with Path('config.json').open('w', encoding='utf-8') as json_file:
      dump({'config': config, 'options': options, 'weight': weight, 'wh': wh}, json_file, indent=2)
