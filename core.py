...
class Model:
  ...
  def predict_image(self, file: UploadedFile) -> None:
    image(
      self.predict(
...
class Annotator:
  def __init__(self, model: Model, config: dict | None = None, wh: tuple[int, int] = (640, 480)) -> None:
    if config is None:
      config = {}
  ...
  @classmethod
  def load(cls, path: str) -> 'Annotator':
